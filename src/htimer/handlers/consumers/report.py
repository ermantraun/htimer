...
from htimer.application.reports import dto, interactors
...
class CreateReportConsumer(BaseConsumer):
    ...
    async def handle(self):
        ...
        async with self.container() as request_scope:
            report_repository = await request_scope.get(
                common_interfaces.ReportRepository
            )
            report_interactor = await request_scope.get(
                interactors.CreateReportInteractor
            )
            ...
            queue = await channel.get_queue(
                config.rabbitmq.report_queue_name, ensure=True
            )
            exchange = await channel.get_exchange(
                config.rabbitmq.exchange_name + "_dlx", ensure=True
            )
            exchange_dlx = await channel.get_exchange(
                config.rabbitmq.exchange_name + "_dlx", ensure=True
            )
                        ...
                        if msg_id is None:
                            if not await try_retry(exchange, message, config):
                                ...
                                await logger.info(
                                    ...
                                    message="msg_id: unknown_id, error: Missing message_id",
                                )

                            continue

                        try:
                            report = await report_repository.get_by_uuid(
                                report_uuid=UUID(msg_id)
                            )
                            ...
                            await report_interactor.execute(
                                dto.CreateReportInDTO(report_id=msg_id)
                            )

                            await message.ack()
                            await logger.info(
                                operation="report_generation_success", message=msg_id
                            )
                            ...
                            if not await try_retry(exchange, message, config):
                                ...
                                exc = await report_repository.update(
                                    report_uuid=UUID(msg_id),
                                    data={"status": entities.ReportStatus.FAILED},
                                )

                                if isinstance(exc, Exception):
                                    await logger.info(
                                        operation="report_status_update_failed",
                                        message=f"msg_id: {msg_id}, error: {str(e)}, details: Cant update report status to FAILED after exhausting retries",
                                    )

                                await logger.info(
                                    operation="report_generation_failed",
                                    message=f"msg_id: {msg_id}, error: {str(e)}",
                                )
                else:
