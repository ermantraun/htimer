...
class Auth(common_interfaces.Context, interfaces.TokenGenerator):
    ...
    async def generate(self, user_uuid: UUID) -> str:
        payload: dict[str, Any] = {
            ...
            "exp": await self.clock.now()
            + timedelta(hours=self.config.expiration_days),
        }
    ...
    def get_current_user_uuid(self) -> UUID | InvalidTokenError:
        if hasattr(self.context, "get_current_user_uuid") and callable(
            self.context.get_current_user_uuid
        ):
            value = self.context.get_current_user_uuid()
        ...
        if hasattr(self.context, "get") and callable(self.context.get):
            ...
            value = self.context.current_user_uuid
        elif hasattr(self.context, "user_uuid"):
            value = self.context.user_uuid
        ...
        if isinstance(value, str):
            try:
                ...
                return InvalidTokenError(
                    "Некорректный идентификатор пользователя в контексте."
                )
