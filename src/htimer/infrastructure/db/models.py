...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(
        sq.Enum(UserRole, values_callable=get_enum_values),
        nullable=False,
    )
    status: Mapped[UserStatus] = mapped_column(
        sq.Enum(UserStatus, values_callable=get_enum_values),
        nullable=False,
    ...
    memberships: Mapped[list[MemberShip]] = relationship(
        ...
        back_populates="user",
        cascade="all, delete-orphan",
        foreign_keys="[MemberShip.user_uuid]",
        passive_deletes=True,
        lazy="raise",
    )
    assigned_memberships: Mapped[list[MemberShip]] = relationship(
        "MemberShip",
        back_populates="assigned_by_user",
        cascade="all, delete-orphan",
        foreign_keys="[MemberShip.assigned_by_uuid]",
        passive_deletes=True,
        lazy="raise",
    )
...
class Project(Base):
    ...
    creator_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("users.uuid", ondelete="CASCADE"), nullable=False
    )
    creator: Mapped[User] = relationship(
        "User", back_populates="owned_projects", lazy="raise"
    )

    start_date: Mapped[date | None] = mapped_column(sq.Date)
    end_date: Mapped[date | None] = mapped_column(sq.Date)
    status: Mapped[ProjectStatus] = mapped_column(
        sq.Enum(ProjectStatus, values_callable=get_enum_values),
        nullable=False,
...
class MemberShip(Base):
    ...
    user_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("users.uuid", ondelete="CASCADE")
    )
    project_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("projects.uuid", ondelete="CASCADE")
    )
    joined_at: Mapped[date] = mapped_column(sq.Date, nullable=False)
    assigned_by_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("users.uuid", ondelete="CASCADE")
    )

    user: Mapped[User] = relationship(
        "User", back_populates="memberships", foreign_keys=[user_uuid], lazy="raise"
    )
    project: Mapped[Project] = relationship(
        "Project", back_populates="memberships", lazy="raise"
    )
    assigned_by_user: Mapped[User] = relationship(
        "User", foreign_keys=[assigned_by_uuid], lazy="raise"
    )

    __table_args__ = (
        sq.UniqueConstraint(
            "project_uuid", "user_uuid", name="uq_memberships_user_project"
        ),
    )
...
class Subscription(Base):
    ...
    uuid: Mapped[UUID] = mapped_column(sq.UUID(as_uuid=True), primary_key=True)
    project_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("projects.uuid", ondelete="CASCADE"), nullable=False
    )
    project: Mapped[Project] = relationship(
        "Project", back_populates="subscription", lazy="raise"
    )
    ...
    status: Mapped[SubscriptionStatus] = mapped_column(
        sq.Enum(SubscriptionStatus, values_callable=get_enum_values),
        default=SubscriptionStatus.UNACTIVE,
    )
...
class Payment(Base):
    ...
    uuid: Mapped[UUID] = mapped_column(sq.UUID(as_uuid=True), primary_key=True)
    subscription_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("subscriptions.uuid", ondelete="CASCADE"), nullable=False
    )
    subscription: Mapped[Subscription] = relationship(
        "Subscription", back_populates="payments", lazy="raise"
    )

    amount: Mapped[float] = mapped_column(sq.Float, nullable=False)
    currency: Mapped[CurrencyEnum] = mapped_column(
        sq.Enum(CurrencyEnum, values_callable=get_enum_values),
        nullable=False,
        default=CurrencyEnum.RUB,
    )
    status: Mapped[PaymentStatus] = mapped_column(
        sq.Enum(PaymentStatus, values_callable=get_enum_values),
        nullable=False,
...
class Stage(Base):
    ...
    project: Mapped[Project] = relationship(
        "Project", back_populates="stages", lazy="raise"
    )
    ...
    parent: Mapped[Stage | None] = relationship(
        "Stage", remote_side="Stage.uuid", back_populates="children", lazy="raise"
    )
    children: Mapped[list[Stage]] = relationship(
        "Stage",
    ...
    status: Mapped[StageStatus] = mapped_column(
        sq.Enum(StageStatus, values_callable=get_enum_values),
    ...
    __table_args__ = (
        ...
        sq.Index(
            "parent_uuid_main_path_idx",
            "parent_uuid",
            unique=True,
            postgresql_where=(sq.column("main_path") == True),
        ),
    )
...
class Task(Base):
    ...
    substage_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("stages.uuid", ondelete="CASCADE")
    )
    substage: Mapped[Stage] = relationship(
        "Stage", back_populates="tasks", lazy="raise"
    )

    created_at: Mapped[date] = mapped_column(sq.Date, nullable=False)
    status: Mapped[TaskStatus] = mapped_column(
        sq.Enum(TaskStatus, values_callable=get_enum_values), default=TaskStatus.PENDING
    )
...
class DailyLog(Base):
    ...
    creator: Mapped[User] = relationship(
        "User", back_populates="daily_logs", lazy="raise"
    )
    ...
    project: Mapped[Project] = relationship(
        "Project", back_populates="daily_logs", lazy="raise"
    )

    substage_uuid: Mapped[UUID | None] = mapped_column(sq.ForeignKey("stages.uuid"))
    substage: Mapped[Stage | None] = relationship(
        "Stage", back_populates="daily_logs", lazy="raise"
    )

    files: Mapped[list[DailyLogFile]] = relationship(
        "DailyLogFile",
    ...
    __table_args__ = (
        sq.UniqueConstraint(
            ...
            "project_uuid",
            "created_at",
            name="uq_creator_project_dailylog_date",
        ),
    )
...
class DailyLogFile(Base):
    ...
    daily_log_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("daily_logs.uuid", ondelete="CASCADE")
    )
    daily_log: Mapped[DailyLog] = relationship(
        "DailyLog", back_populates="files", lazy="raise"
    )
...
class UserReport(Base):
    ...
    report_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("reports.uuid", ondelete="CASCADE"), nullable=False
    )
    report: Mapped[Report] = relationship(
        "Report", lazy="raise", back_populates="target_users"
    )
...
class Report(Base):
    ...
    project: Mapped[Project] = relationship(
        "Project", lazy="raise", back_populates="reports"
    )

    generated_by_uuid: Mapped[UUID] = mapped_column(
        sq.ForeignKey("users.uuid", ondelete="CASCADE"), nullable=False
    )
    ...
    status: Mapped[ReportStatus] = mapped_column(
        sq.Enum(ReportStatus, values_callable=get_enum_values),
