...
class ProjectBlockedError(ProjectAuthorizationError):
    ...
    def __init__(self, message: str = "Недостаточно прав: операция недоступна для неактивного проекта."):
        super().__init__(message)

class UserNotCreatorError(ProjectAuthorizationError):
    """Ошибка авторизации: пользователь не является создателем проекта."""

    def __init__(self, message: str = "Недостаточно прав: операция доступна только создателю проекта."):
        super().__init__(message)
