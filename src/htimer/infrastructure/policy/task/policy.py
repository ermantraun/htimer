...
class TaskAuthorizationPolicyImpl(interfaces.TaskAuthorizationPolicy):
    def decide_create_task(
        ...
        if (
            decision
            is entities.UserDecisions.CreateTaskDecision.FORBIDDEN_FOR_NON_MEMBER
        ):
            return exceptions.UserNotProjectMemberError(
                "Недостаточно прав: пользователь не является участником проекта."
            )
    ...
    def decide_update_task(
        ...
        if (
            decision
            is entities.UserDecisions.UpdateTaskDecision.FORBIDDEN_FOR_NON_MEMBER
        ):
            return exceptions.UserNotProjectMemberError(
                "Недостаточно прав: обновление задачи доступно только участнику проекта."
            )
    ...
    def decide_get_task(
        self,
        actor: entities.User,
        task_project: entities.Project,
        task_project_members: list[entities.User],
    ) -> exceptions.UserNotProjectMemberError | None:
        ...
        if decision is entities.UserDecisions.GetTaskDecision.FORBIDDEN_FOR_NON_MEMBER:
            return exceptions.UserNotProjectMemberError(
                "Недостаточно прав: просмотр задачи доступен только участнику проекта."
            )
    ...
    def decide_delete_task(
        ...
        if (
            decision
            is entities.UserDecisions.DeleteTaskDecision.FORBIDDEN_FOR_NON_MEMBER
        ):
            return exceptions.UserNotProjectMemberError(
                "Недостаточно прав: удаление задачи доступно только участнику проекта."
            )
