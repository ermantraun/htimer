...
class SystemClock(common_interfaces.Clock):
    ...
    def verify_date(self, date: str) -> str | InvalidDate:
        try:
            ...
            return InvalidDate(
                f"Некорректный формат даты: {date}. Ожидается формат YYYY-MM-DD."
            )

    def verify_period(
        self,
        start_date: common_interfaces.date | None,
        end_date: common_interfaces.date | None,
    ) -> None | InvalidDate:
        if start_date and end_date:
            ...
            return InvalidDate(
                "Дата начала и дата окончания должны быть указаны вместе."
            )
