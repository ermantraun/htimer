...
class CreateReportRequestInteractor:
    def __init__(
        ...
        report_repository: common_interfaces.ReportRepository,
        project_repository: common_interfaces.ProjectRepository,
        job_gateway: common_interfaces.JobGateway,
        context: common_interfaces.Context,
        authorization_policy: interfaces.ReportsAuthorizationPolicy,
    ):
    ...
    async def execute(
        self, data: dto.CreateReportRequestInDTO
    ) -> dto.CreateReportRequestOutDTO:
        ...
        if data.start_date or data.end_date:
            verify_period_error = self.clock.verify_period(
        ...
        authorization_error = self.authorization_policy.decide_create_report(
            actor, project, project_members, users
        )
        ...
        if isinstance(
            enqueue_error,
            (common_exceptions.RepositoryError, common_exceptions.JobGatewayError),
        ):
            raise enqueue_error

        await self.logger.info(
            message=f"Report {report.uuid} creation requested by user {actor.uuid} for project {project.uuid} with period {start_date} to {end_date}",
            operation="create_report_request",
        )
...
class CreateReportInteractor:
    def __init__(
        ...
        visualizer: interfaces.ReportVizualizer,
        project_repository: common_interfaces.ProjectRepository,
        report_repository: common_interfaces.ReportRepository,
        logger: common_interfaces.Logger,
    ...
    async def execute(self, dto: dto.CreateReportInDTO) -> None:
        ...
        if isinstance(
            report,
            (common_exceptions.ReportNotFoundError, common_exceptions.RepositoryError),
        ):
            await self.logger.info(
                message=f"Report generation failed for report {dto.report_id} due to report not found",
                operation="report_generation_failed",
            )
            raise report
        ...
        if isinstance(
            ...
            await self.logger.info(
                message=f"Report generation failed for report {dto.report_id} due to project not found",
                operation="report_generation_failed",
            )
            raise project_members
        ...
        daily_logs = await self.daily_log_repository.get_list_by_project(
            ...
            report.start_date,
            report.end_date,
            [member.uuid for member in project_members],
            draft=False,
        )
        ...
        if isinstance(
            ...
            await self.logger.info(
                message=f"Report generation failed for report {dto.report_id} due to error fetching daily logs",
                operation="report_generation_failed",
            )
            raise daily_logs
        ...
        if report.is_summary_report():
            ...
            if isinstance(
                ...
                await self.logger.info(
                    message=f"Report generation failed for report {dto.report_id} due to error fetching tasks",
                    operation="report_generation_failed",
                )
                raise tasks
        ...
        report_update_error = await self.report_repository.update(
            report.uuid, {"status": report.status, "file_name": report.file_name}
        )

        if isinstance(
            report_update_error,
            (common_exceptions.RepositoryError, common_exceptions.ReportNotFoundError),
        ):
            await self.logger.info(
                message=f"Report generation failed for report {dto.report_id} due to error updating report status",
                operation="report_generation_failed",
            )
            raise report_update_error

        error = await self.file_storage.save(
            file_name=f"{report.uuid}", content=visualization
        )
        ...
        if isinstance(
            ...
            await self.logger.info(
                message=f"Report generation failed for report {dto.report_id} due to error saving report file",
                operation="report_generation_failed",
            )
            raise error
