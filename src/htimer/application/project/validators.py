...
class CreateProjectValidator:
    ...
    def validate(
        self, data: dto.CreateProjectInDTO
    ) -> (
        None
...
class GetProjectListValidator:
    ...
    def validate(
        self, dto_data: dto.GetProjectListInDTO
    ) -> exceptions.ProjectValidationError | None:
        return None
...
def _validate_name(name: str) -> exceptions.InvalidProjectNameError | None:
    ...
    if not name or not name.strip():
        return exceptions.InvalidProjectNameError(
...
def _validate_description(
    description: str,
) -> exceptions.InvalidProjectDescriptionError | None:
    if len(description) > 500:
        return exceptions.InvalidProjectDescriptionError(
            "Описание проекта не может превышать 500 символов."
        )
    return None
