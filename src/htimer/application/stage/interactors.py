...
class CreateStageInteractor:
    ...
    async def execute(self, data: dto.CreateStageInDTO) -> dto.CreateStageOutDTO:
        ...
        data.description = (
            self.text_normalizer.normalize(data.description)
            if data.description
        ...
        parent_stage = (
            await self.stage_repository.get_by_uuid(data.parent_uuid)
            if data.parent_uuid
            else None
        )
        ...
        authorization_error = self.authorization_policy.decide_create_stage(
...
class UpdateStageInteractor:
    ...
    async def execute(self, data: dto.UpdateStageInDTO) -> dto.UpdateStageOutDTO:
        ...
        authorization_error = self.authorization_policy.decide_update_stage(
            actor, stage.project, project_members
        )
        if authorization_error is not None:
            raise authorization_error
