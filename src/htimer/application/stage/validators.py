...
class CreateStageValidator:
    ...
    def validate(
        self, data: dto.CreateStageInDTO
    ) -> (
        None
...
class UpdateStageValidator:
    ...
    def validate(
        self, data: dto.UpdateStageInDTO
    ) -> (
        None
        | exceptions.InvalidStageNameError
        | exceptions.InvalidStageDescriptionError
...
def _validate_description(
    description: str,
) -> exceptions.InvalidStageDescriptionError | None:
    if len(description) > 500:
        return exceptions.InvalidStageDescriptionError(
            "Описание этапа не может превышать 500 символов."
        )
    return None
